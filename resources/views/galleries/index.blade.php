    ...
    <div x-data="{
        ...
        <div class="max-w-[1400px] mx-auto px-4 sm:px-8 py-12 sm:py-20 relative z-10">
            ...
            <div x-show="showModal" 
                 ...
                 <!-- Slider Container Utama (Diperbaiki padding agar tidak menabrak header & thumbnail) -->
                 <div class="relative w-full h-full flex items-center justify-center px-4 sm:px-12 pt-24 sm:pt-28 pb-32 sm:pb-40" @click.self="closeGallery()">
                     
                     <!-- Tombol Navigasi Kiri (Prev) -->
                     <button @click.stop="prev()" 
                             x-show="activeImages.length > 1"
                             class="absolute top-1/2 -translate-y-1/2 left-2 sm:left-8 z-50 p-3 sm:p-4 rounded-full bg-black/50 text-white hover:bg-red-600 transition-all border border-neutral-700 backdrop-blur group">
                     ...
                     <button @click.stop="next()" 
                             x-show="activeImages.length > 1"
                             class="absolute top-1/2 -translate-y-1/2 right-2 sm:right-8 z-50 p-3 sm:p-4 rounded-full bg-black/50 text-white hover:bg-red-600 transition-all border border-neutral-700 backdrop-blur group">
                         <svg class="w-6 h-6 sm:w-8 sm:h-8 transform group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                 ...
                 <div class="absolute bottom-6 left-0 w-full px-4 z-[10000]" x-show="activeImages.length > 1">
