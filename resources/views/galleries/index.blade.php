<x-layouts.app :settings="$settings ?? null">
    ...
    <div x-data="{
        ...
        class="max-w-[1400px] mx-auto px-4 sm:px-8 py-12 sm:py-20 relative">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            @foreach($galleries as $gallery)
                @php
                    // Logika Ekstraksi Gambar:
                    // Asumsi kolom 'images' berisi array nama file foto. 
                    // Kita gabungkan dengan cover_image jika cover image tidak ada di list 'images'.
                    $rawImages = is_string($gallery->images) ? json_decode($gallery->images, true) : ($gallery->images ?? []);
                    $imagesCollection = collect($rawImages);
                    
                    if ($gallery->cover_image && !$imagesCollection->contains($gallery->cover_image)) {
                        $imagesCollection->prepend($gallery->cover_image);
                    }

                    // Ubah semua path menjadi full URL Asset
                ...
                <button type="button" 
                        ...
                        class="group text-left flex flex-col h-full w-full bg-white dark:bg-[#121212] rounded-3xl overflow-hidden border border-neutral-100 dark:border-neutral-800 shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 relative">
                    
                    <div class="relative aspect-[4/5] overflow-hidden w-full">
                    ...
                    <div class="p-6 flex-1 flex flex-col">
                        <span class="text-[10px] font-black text-red-600 uppercase tracking-widest mb-3 block">
                            {{ $gallery->published_at ? $gallery->published_at->format('d M Y') : 'Baru Saja' }}
                        </span>
             ...
             <div class="relative w-full h-full flex items-center justify-center p-4 sm:p-12 pb-24 sm:pb-32" @click.self="closeGallery()">
                 ...
                 <template x-if="activeImages.length > 0">
                     <img :src="activeImages[currentIndex]" 
                          ...
                          :key="currentIndex"> {{-- ':key' memicu animasi saat index berubah --}}
                 </template>
