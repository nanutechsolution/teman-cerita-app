    ...
    <div class="max-w-[1400px] mx-auto px-0 sm:px-4 lg:px-8 pt-0 sm:pt-6 pb-24">
        ...
        <section x-data="galleryLightbox()" class="mb-16 px-4 sm:px-0">
            <div class="flex items-center gap-4 border-b-[3px] border-black dark:border-white pb-3 mb-6">
                ...
                <h2 class="text-2xl sm:text-3xl font-black text-neutral-900 dark:text-white uppercase tracking-tighter">Lensa Flobamora</h2>
            ...
            <template x-teleport="body">
                <div x-show="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/98 backdrop-blur-2xl p-4 sm:p-10" x-cloak @keydown.escape.window="closeModal()" @keydown.right.window="nextImage()" @keydown.left.window="prevImage()">
                    <button @click="closeModal()" class="absolute top-6 right-6 z-[110] text-white/50 hover:text-white p-3 bg-white/5 hover:bg-red-600 rounded-full"><svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
                    <button @click="prevImage()" class="hidden sm:flex absolute left-8 z-[110] p-5 text-white/40 hover:text-white"><svg class="w-12 h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path d="M15 19l-7-7 7-7" /></svg></button>
                    <button @click="nextImage()" class="hidden sm:flex absolute right-8 z-[110] p-5 text-white/40 hover:text-white"><svg class="w-12 h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path d="M9 5l7 7-7 7" /></svg></button>
        ...
        <section id="indeks-terkini" class="px-4 sm:px-0">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-10">
                ...
                @empty
                @for ($i = 0; $i < 8; $i++)
                <article class="group flex flex-col h-full animate-pulse"><div class="aspect-[4/3] bg-neutral-200 dark:bg-neutral-800 mb-3 w-full rounded-xl"></div><div class="h-4 bg-neutral-200 dark:bg-neutral-800 w-1/4 mb-2"></div></article>
                @endfor
                @endforelse
            </div>
        </section>
    ...
    <script>
        function galleryLightbox() {
            return {
                ...
                nextImage() { this.currentIndex = (this.currentIndex + 1) % this.activeImages.length; },
                prevImage() { this.currentIndex = (this.currentIndex - 1 + this.activeImages.length) % this.activeImages.length; },
                scrollNext() { document.getElementById('gallery-container').scrollBy({ left: 400, behavior: 'smooth' }); },
                scrollPrev() { document.getElementById('gallery-container').scrollBy({ left: -400, behavior: 'smooth' }); }
            }
        }
    </script>
    @endpush
