<x-layouts.app :settings="$settings ?? null">
    ...
    <div x-data="{ showModal: false, activeVideo: null }" class="min-h-screen bg-neutral-50 dark:bg-black py-12 sm:py-16 relative overflow-hidden transition-colors duration-300">
            ...
            @if(isset($videos) && $videos->count() > 0)
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8">
                    @foreach($videos as $video)
                        @php
                            // Logika SUPER TANGGUH untuk mengubah semua Link YouTube menjadi Link Embed
                            $embedUrl = $video->link;
                            $hasVideoUrl = !empty($embedUrl);
                            
                            if ($hasVideoUrl) {
                                // Regex ini akan menangkap ID video (11 karakter) dari format Watch, youtu.be, Embed, maupun Shorts
                                if (preg_match('/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?|shorts)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s]{11})/i', $embedUrl, $matches)) {
                                    $videoId = $matches[1];
        ...
        <div x-show="showModal" 
             ...
             {{-- Kontainer Iframe Video (Support 16:9 dan 9:16 Shorts) --}}
